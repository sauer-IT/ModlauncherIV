...
namespace mliv
{
    ...
    std::string Config::DefaultText()
    {
        return
            ...
            "#\n"
            "# Navigation is on the d-pad. D-pad up is also the phone in GTA IV,\n"
            "# so while the menu is open the game does not get to see the buttons\n"
            "# bound here - scrolling no longer brings the phone up. Sticks,\n"
            "# triggers and every other button still reach the game, so walking\n"
            "# and driving with the menu open keep working. A button still held\n"
            "# when the menu closes stays with the menu until it is let go.\n"
            "#\n"
            "# The sticks can be bound instead - LStickUp and the rest. A stick\n"
            "# used here is then kept from the game too while the menu is open.\n"
            "[Pad]\n"
            "Enabled = yes\n"
            "Menu    = L3+R3\n"
            "Up      = DPadUp\n"
